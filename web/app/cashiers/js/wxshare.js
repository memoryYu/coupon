	...
	var wxshare = window.wxshare = {
		...
		initWxCfg : function(share, letwxid, apiopenid, apitoken, wxconfig, succCb, cancelCb, errorCb, readyCb){
			...
			wx.config({
				...
				jsApiList: ['onMenuShareTimeline', 'onMenuShareAppMessage', 'onMenuShareQQ', 'onMenuShareWeibo','hideOptionMenu','showOptionMenu','scanQRCode']
			});
			wx.ready(function() {
				readyCb && readyCb(wx);
...
//					localStorage.scancode = result;
//				}
//			});
		},
		
		//微信扫一扫接口
		scanQRCode : function(succCb, cancelCb, errorCb, readyCb){
			wx.scanQRCode({
				needResult: 1, // 默认为0，扫描结果由微信处理，1则直接返回扫描结果，
    			scanType: ["qrCode","barCode"], // 可以指定扫二维码还是一维码，默认二者都有
				success: function(res) {
